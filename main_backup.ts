    ...
    if (pathname === "/upload" && req.method === "POST") {
        try {
            ...
            if (!images || !Array.isArray(images) || images.length === 0) {
                return createJsonErrorResponse("No images provided", 400);
            }
            ...
            for (const base64Data of images) {
                try {
                    if (imgbbApiKey) {
                        // 使用 ImgBB 上传
                        console.log("[Upload] 使用 ImgBB 上传图片");
                        ...
                        if (response.ok) {
                            ...
                            if (result.success && result.data?.url) {
                                ...
                                console.log(`[Upload] ImgBB 上传成功: ${result.data.url}`);
                                continue;
                            }
                        }
                        
                        console.log("[Upload] ImgBB 上传失败，回退到本地存储");
                    }
                    
                    // 回退到本地存储（但在 Deno Deploy 中不可用）
                    console.log("[Upload] 警告: 本地存储在 Deno Deploy 中不可用");
                    return createJsonErrorResponse("图片上传失败: 需要配置 IMGBB_API_KEY", 500);
